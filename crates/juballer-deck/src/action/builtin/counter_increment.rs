//! counter.increment action — bump a named counter in state + publish new value.
//!
//! Args:
...
impl BuildFromArgs for CounterIncrement {
    fn from_args(args: &toml::Table) -> Result<Self> {
        let name = args
            .get("name")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Error::Config("counter.increment requires name".into()))?
            .to_string();
...
impl Action for CounterIncrement {
    fn on_down(&mut self, cx: &mut ActionCx<'_>) {
        ...
        let cur = cx
            ...
            .and_then(|v| v.as_i64())
            .unwrap_or(0);
        let new = cur.saturating_add(self.step);
...
mod tests {
    ...
    fn from_args_requires_name() {
        let err = CounterIncrement::from_args(&toml::Table::new()).unwrap_err();
        match err {
            Error::Config(_) => {}
    ...
    fn from_args_accepts_name_default_step() {
        ...
        args.insert("name".into(), toml::Value::String("hits".into()));
        let a = CounterIncrement::from_args(&args).unwrap();
        assert_eq!(a.name, "hits");
        assert_eq!(a.step, 1);
    }
    ...
    fn from_args_accepts_custom_step() {
        ...
        args.insert("step".into(), toml::Value::Integer(5));
        let a = CounterIncrement::from_args(&args).unwrap();
        assert_eq!(a.step, 5);
    }
}
